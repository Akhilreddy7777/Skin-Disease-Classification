# -*- coding: utf-8 -*-
"""
creating a network for training Resnet on SD-198 dataset
...
Classes = 198
Note: Since reworking Densenet code might find densenet instead of 
resnet in a couple of places

>todo: Training now! on cuda 5
"""
...
if __name__ == '__main__':
    ...
    densenet = torchvision.models.resnet18(pretrained=False)
    ...
    torch.save(densenet.state_dict(),'./models/sd198_resnet.pth')
    ...
    print('The testing accuracy for sd198 on resnet is',accuracy)
    with torch.no_grad():
        for data in train_loader:
            inputs,labels = data
            inputs,labels = Variable(inputs),Variable(labels)
    ...
    print('The training accuracy for sd198 on resnet is',accuracy)

    loss_save = np.array(loss_curve)
    np.save('loss_curve_resnet_sd198.npy',loss_save)
#    epochs = np.arange(2)
